...
type TestValue {
  TestInputOutput(
    ...
    ///
    /// ```gleam
    /// use r <- promise.await(queries.select_one(db))
    /// let assert Ok(sql.Returned([result], _)) = r
...
const integration_tests = [
  ...
  // Array
  //
  // > ⚠️ Array *parameters* are not tested here: passing an array argument to a
  // > parameterised query is currently broken in Bun (it fails with
  // > "insufficient data left in message", even using Bun's own `sql.array`).
  // > Array *decoding* works fine. See the README for details.
  //
  // Custom enums
  TestType(
    "squirrel_colour",
    [
      TestValue("queries.LightBrown"),
      TestValue("queries.Red"),
      TestValue("queries.Grey"),
    ],
  ),
  // Json
  TestType(
  ...
  TestType(
    ...
    [
      TestInputOutput(
        input: "json.object([#(\"a\", json.int(1))])",
        // Bun parses `jsonb` into a JavaScript object which we serialize back
        // with `JSON.stringify`, so the result is compact (no spaces).
        output: "\"{\\\"a\\\":1}\"",
      ),
    ],
  ),
...
pub fn integration_test_() {
  // This test takes a while since it has to download dependencies and compile
  // an entire gleam project (and run it on Bun). So we have to increase the
  // timeout.
  use <- with_timeout(seconds: 120)
...
fn create_query_files_and_assertions(
  ...
  let assertions = {
    ...
    "
<heading>
use r <- promise.await(queries.<insert>(db, input))
let assert Ok(sql.Returned([], 1)) = r
use r <- promise.await(queries.<select_all>(db))
let assert Ok(sql.Returned([res], 1)) = r
...
use r <- promise.await(queries.<delete>(db))
let assert Ok(sql.Returned([], 1)) = r
"
...
fn create_query_files_and_assertions_for_query(
  ...
  "use r <- promise.await(queries.<query>(db))
let assert Ok(_) = r"
...
pub fn main() {
  let assert Ok(db) =
    sql.default_config()
    |> sql.port(5432)
    |> sql.user(\"squirrel_test\")
    |> sql.host(\"localhost\")
    |> sql.database(\"squirrel_test\")
    |> sql.connect
...
fn test_project(dir: String) -> Result(String, #(Int, String)) {
  // Squirrel itself runs on the Erlang target (it talks to Postgres over its
  // own protocol using `mug`), so we generate the code with `--target erlang`
  // even though the project it generates code for targets JavaScript/Bun.
  let assert Ok(_) =
    shellout.command(
      run: "gleam",
      with: ["run", "-msquirrel", "--target", "erlang"],
      in: dir,
      opt: [],
    )

  // The generated code and `main` run on the JavaScript target under Bun, as
  // configured in the project's `gleam.toml`. We force `TZ=UTC` so that
  // `timestamp` (without time zone) values round-trip exactly: Bun decodes them
  // through a JavaScript `Date`, which is otherwise shifted by the runtime's
  // local timezone.
  shellout.command(
    run: "env",
    with: ["TZ=UTC", "gleam", "run", "-mmain"],
    in: dir,
    opt: [],
  )
}
