...
/// Creates all the tables, types and extensions the tests run their queries
/// against. Squirrel talks to Postgres over its own protocol, but for the test
/// fixtures we just shell out to `psql`.
///
fn setup_database() {
  let assert Ok(_) =
    shellout.command(
      run: "psql",
      with: [database_url(), "-v", "ON_ERROR_STOP=1", "-c", setup_sql],
      in: ".",
      opt: [],
    )
  Nil
...
fn should_codegen_queries_options(
  ...
  let code =
    query.generate_code("v-test", for: queries, from: "./test-directory").code
