...
///
/// > ⚠️ The generated code runs on the JavaScript target under the
/// > [Bun](https://bun.com) runtime and relies on the
/// > [`brioche`](https://hexdocs.pm/brioche/) package (which wraps Bun's
/// > built-in SQL client). Make sure your project sets `target = "javascript"`
/// > with `runtime = "bun"` and depends on `brioche`, `gleam_javascript`,
/// > `gleam_time`, `youid` and `gleam_json`.
/// >
/// > Squirrel itself runs on the Erlang target, so if your project targets
/// > JavaScript run the generator with `gleam run -m squirrel --target erlang`.
///
pub fn main() {
  case parse_cli_args() {
    Error(Nil) -> {
...
fn write_queries_to_file(
  ...
  let query.Generated(code:, ffi:) =
    query.generate_code(squirrel_version, for: queries, from: queries_directory)

  use _ <- result.try(safely_overwrite(to: file, content: code))
  // Some queries decode types that Bun returns as JavaScript values (`date`,
  // `json`/`jsonb`), which need a companion `sql_ffi.mjs` file next to the
  // generated module.
  use _ <- result.try(case ffi {
    option.None -> Ok(Nil)
    option.Some(ffi) -> {
      let ffi_file = filepath.join(directory, "sql_ffi.mjs")
      simplifile.write(ffi, to: ffi_file)
      |> result.map_error(CannotWriteToFile(ffi_file, _))
    }
  })
...
fn check_queries_code(
  ...
  let expected_code =
    query.generate_code(squirrel_version, for: queries, from: "check-queries").code
